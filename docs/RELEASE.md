...
AGENT_ORB_VERSION=v0.1.16 npx @solar_orb/agent_orb
```

By default, the bootstrapper downloads the GitHub Release tag matching its own npm package version, for example npm `0.1.16` downloads release `v0.1.16`. If `github_repository` is empty and no override is supplied, setup will fall back to local bundled assets or source build.
