...
import { codexHooksInstalled, codexHooksPath, enableCodexHooksFeature, installCodexHooks, removeCodexHooks, } from './codex-hooks.js';
 ...
 */
export async function uninstall(platform = detectPlatform()) {
    printHeader(platform);
    assertSupported(platform);
    cleanupInstalledRuntime(platform);
...
async function configureCodexHooks(platform, adapters, yes = false) {
...
function findRepoRoot(platform) {
    ...
    if (fs.existsSync(path.join(fromPackage, 'Cargo.toml')))
        return fromPackage;
    throw new Error(`No prebuilt runtime bundle is available for ${platform.platform}/${platform.arch}, and the Agent Orb source checkout was not found. ` +
