...
export async function doctor(platform = detectPlatform()): Promise<void> {
  printHeader(platform);
  assertSupported(platform);
...
function ensurePathHint(platform: PlatformInfo): void {
  const currentPath = process.env.PATH ?? '';
  const parts = currentPath.split(platform.pathDelimiter).filter(Boolean);
  if (parts.includes(platform.runtimeDir)) return;
...
async function ensureDaemon(platform: PlatformInfo): Promise<void> {
  console.log('\n==> Starting daemon');
  ...
  if (daemonAlreadyHealthy) {
    throw new Error(
      `agent_orbd is already running on 127.0.0.1:17321, but it does not accept the token at ${tokenPath}. Stop the existing agent_orbd process and rerun setup.`,
    );
  }
...
async function authenticatedStatus(tokenPath: string): Promise<boolean> {
  try {
