...
import { hasPrebuiltRuntimeBundle, type PlatformInfo } from './platform.js';
 ...
 */
export function runtimeMatchesExpectedVersion(platform: PlatformInfo): boolean {
  if (!runtimeLooksInstalled(platform)) return false;

  const expected = readPackageVersion();
  if (!expected) return runtimeLooksInstalled(platform);
...
export function installedRuntimeVersion(platform: PlatformInfo): string | undefined {
  const exe = path.join(platform.runtimeDir, `agent_orb${platform.exeSuffix}`);
  if (!fs.existsSync(exe)) return undefined;
  try {
...
export function runtimeSupportsSubcommand(platform: PlatformInfo, subcommand: string): boolean {
  const exe = path.join(platform.runtimeDir, `agent_orb${platform.exeSuffix}`);
...
function releaseBaseUrl(platform: PlatformInfo, options: BundleInstallOptions): string | undefined {
  ...
  if (!hasPrebuiltRuntimeBundle(platform.platform, platform.arch)) return undefined;
