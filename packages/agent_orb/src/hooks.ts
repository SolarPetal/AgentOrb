 ...
 *   parse.
 */
export function installClaudeHooks(agentOrbExe: string): HookInstallResult {
 ...
 */
export function removeClaudeHooks(): HookInstallResult {
  const settingsPath = claudeSettingsPath();
  const existing = readSettings(settingsPath);
...
export function claudeHooksInstalled(): boolean {
...
function readSettings(settingsPath: string): ReadSettings {
  ...
  try {
    ...
    throw new Error(
      `Refusing to modify ${settingsPath}: it is not valid JSON (${message}). ` +
